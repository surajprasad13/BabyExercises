...
class VideoScreen extends Component {
    ...
    async componentWillMount() {
        ...
        this.setState({ video: videoData, isDownloaded: isDownloaded, isLoading: false, file: file });
    }
    /**
     * Do download
     * 
    ...
    async videoClick() {
        ...
        if (this.state.isDownloaded) {
            ...
            else { //Go to video player screen if Android
                this.props.navigation.navigate("VideoPlayer", { videoPath: this.state.file, video: this.state.video })
            }
        }
        else {
    ...
    async download(ids) {
        ...
        global.events.trigger('download.next', { num: CommonDataManager.getInstance().getCurrentDownload() })
        let options = {
            fileCache: true,
        ...
        RNFetchBlob.config(options).fetch('GET', Config.API_URL + "/video/" + id, {
            Authorization: "Bearer " + token,
        }).progress((received, total) => {
            global.events.trigger('download.progress', { percent: received / total, total: this.state.totalVideos });
        }).then((res) => {
            AsyncStorage.setItem("video" + id, res.path()); //Save path to video

            if (this.state.video.id == id) { //This video just got downloaded, update its state
            ...
            global.events.trigger('download.progress', { percent: 0, total: this.state.totalVideos }); //Updates progress bar
    ...
    async downloadMultiple(downloadAll = false) {
        ...
        if (downloadAll) {
            ...
            if (allBought != null) { //If you have bought something
       
                if (allBought.includes("videopackage_all")) { //If bought "All Videos category"
                ...
                for (let freeVid of freeVideos) { //Add free video from category you don't own
            ...
            else { //if doesn't own anything - download all free videos               
        ...
        //ALL IN CATEGORY
            ...
            if (allBought != null && allBought.includes(thisCategory)) { //if bought add all to dl list
                videoArr.push(...Array.from(allInCategory));
            }
            else {//if not bought, add free to dl list
        ...
        return (
            <MenuDrawer>
                <View style={styles.wrapper}>
                    {(this.state.file != null && Platform.OS == 'ios') &&
                        <Video source={{ uri: this.state.file }}
            ...
            scrollIndicatorInsets={{right: 1}}>
                        <View style={styles.container}>
                            <TouchableOpacity style={styles.border} onPress={() => this.videoClick(this.state.video.id)}>
                                <ImageBackground resizeMode="cover" style={styles.thumbnail} source={{ uri: Config.URL + this.state.video.image }}>
                                    <Image style={styles.centerImage} source={this.renderImageIcon()} />
                                </ImageBackground>
                            </TouchableOpacity>
                            <View style={styles.textWrap}>
                                <Text style={styles.title}>{this.state.video.title}</Text>
                                <Text style={styles.text}>{this.state.video.description}</Text>
                                <Text style={styles.lengthLabel}>{strings.videoLength}: {this.state.video.duration}</Text>
                            </View>
                        </View>
                    </ScrollView>
                    ...
                    {isTablet &&
                        <View style={styles.sideView}>
                            <VideoSide productId={this.state.video.product_id} navigation={this.props.navigation} exclude={this.state.video.id} />
                        </View>}
                </View>
                ...
                <ActionSheet
                    ...
                    tintColor='#23475f'
                    options={[strings.downloadOnlyThis, strings.downloadAllInCategory, strings.downloadAllVideos, strings.cancel]}
    ...
    renderImageIcon() {
        ...
        if (this.state.isDownloaded) //is downloaded 
        {
