...
export default class Menu extends Component {
    ...
    componentWillMount() {
        try {
            Data.getPages().then(data => {
                this.setState({
                    pages: data.pages.sort(function (a, b) { return (a.sort > b.sort) ? 1 : ((b.sort > a.sort) ? -1 : 0); }),
                    isLoading: false,
                });
            });
        }
    ...
    render() {
        ...
        return (
            <LinearGradient colors={['rgba(134,194,211,0.8)', 'rgba(48,86,108, 0.8)']} style={styles.container}>           
                <ScrollView 
            scrollIndicatorInsets={{right: 1}}>
                    <View style={styles.closeButton}>
                        <TouchableWithoutFeedback onPress={this.props.closeDrawer} >
                            <Image style={styles.closeImage} source={require('./../../../assets/cancel/cancel_icon.png')} />
                        </TouchableWithoutFeedback>
                    </View>
                    ...
                    {this.state.pages.map(pageData => {
                        if (pageData.lang_iso === CommonDataManager.getInstance().getSelectedLanguage().iso) {
                    ...
                    <Image source={require('./../../../assets/baby.png')} style={styles.baby} />
                </ScrollView>
            </LinearGradient>
        )
    }
