...
class PayModal extends Component {
    ...
    async componentWillMount() {
        ...
        let cats = await Data.getCategories(CommonDataManager.getInstance().getSelectedLanguage().iso);
    ...
    render() {     
         return (
                ...
                <Modal style={styles.modal}
                    ...
                    <View style={styles.close}>
                        <TouchableOpacity onPress={this.toggleModal} >
                            <Image source={require("../../../assets/close/close_BTN.png")} style={styles.closeImage} />
                        </TouchableOpacity>
                    </View>
            ...
            scrollIndicatorInsets={{right: 1}}
                            ref={(ref) => { this.payScroll = ref }}
                            onContentSizeChange={(w,h)=>{this.payScroll.scrollToEnd();}}>
                                {this.state.categories != null && //Is loaded
                                Array.from(this.state.categories).map(cat => {
                                if (cat.id) { //loaded
                                    const item = this.state.skus.find(x => x.productId == cat.product_id);
                                    if (item) { //If data is loaded, remove (text), Play Store adds text to the title
                        ...
                        <TouchableOpacity style={styles.restoreButton} onPress={this.restorePurchases}> 
                            <Text style={styles.restoreButtonText}>{strings.restorePuchases}</Text>
                        </TouchableOpacity>
                    </View>
                </Modal>
            </View>
    ...
    renderBuyAll() {
        const itemAll = this.state.skus.find(x => x.productId == "videopackage_all");
        if (itemAll) {
            ...
            <View style={styles.item} key={item.productId}>
                ...
                <View style={styles.itemRow}>
                    ...
                    {!this.state.own.find(x => x == item.productId) &&
                        <TouchableOpacity style={styles.itemButton} onPress={() => this.buyProduct(item.productId)}>
                            <Text numberOfLines={1} style={styles.itemButtonText}>{item.localizedPrice} </Text>
                        </TouchableOpacity>
                    }
                    {this.state.own.find(x => x == item.productId) &&
                        <View style={styles.itemButton}><Text numberOfLines={1} style={styles.itemButtonText}>OK</Text></View>
                    }
                </View>
            </View>
        );
        ...
        try {
            ...
            if (purchases && purchases.length > 0) {
                purchases.forEach(purchase => {
                    ...
                    OneSignal.sendTags({ "have_paid": "true", "last_item": purchase.productId });
    ...
    async buyProduct(id) {
        ...
        RNIap.buyProduct(id).then(purchase => {
