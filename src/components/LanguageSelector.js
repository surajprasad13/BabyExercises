...
export default class LanguageSelector extends Component {
    ...
    render() {
        ...
        return (
            ...
            <ModalDropdown style={{ flex: 1, padding: 15, }}
                dropdownStyle={styles.dropdown}
                onSelect={(idx, data) => this.onLanguageSelect(idx, data)}
                options={this.state.languages}
                renderRow={this.renderLanguageRow.bind(this)}
            >
                {this.renderLanguageSelect(CommonDataManager.getInstance().getSelectedLanguage())}
            </ModalDropdown>
        );
    }
    /**
    ...
    renderLanguageSelect(language) {
        return (
            <Image style={styles.selectedImage} source={{ uri: Config.URL + language.image }} />
        );
    }
    /**
        ...
        return (
            <TouchableOpacity>
                <View style={styles.row}>
                    <Image source={{ uri: Config.URL + rowData.image }} style={styles.image} />
