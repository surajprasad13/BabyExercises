...
export default class VideoSide extends Component {
  ...
  async componentWillMount() {
    ...
    await this.getData();
    this.downloadFinishListenerId = global.events.addListener(
      'download.finish',
      id => {
  ...
  renderItem(item, index) {
    ...
    let banner =
      banners[CommonDataManager.getInstance().getSelectedLanguage().iso];
    if (this.props.exclude != item.id) {
      //Hide video from sidepanel
      return (
        <TouchableWithoutFeedback
          key={item.id}
          onPress={() => this.onVideoPress(item)}>
            ...
            <View style={styles.imageBorder}>
              <ImageBackground
                ...
                <Image
                  ...
                  source={this.renderImageIcon(item)}
                />
